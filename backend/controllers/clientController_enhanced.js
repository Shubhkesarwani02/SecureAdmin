...
const getClients = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (currentUserRole === 'superadmin') {
      ...
      // Admin has NO ACCESS to client management according to requirements
      return res.status(403).json({
        success: false,
        message: 'Access denied. Admins do not have access to client management.'
      });
      
    } else if (currentUserRole === 'csm') {
      // CSM has NO ACCESS to client management according to requirements
      return res.status(403).json({
        success: false,
        message: 'Access denied. CSMs do not have access to client management.'
      });
    }
...
const exportClients = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (currentUserRole === 'superadmin') {
      ...
      // Admin has NO ACCESS to client management
      return res.status(403).json({
        success: false,
        message: 'Access denied. Admins do not have access to client management.'
      });
